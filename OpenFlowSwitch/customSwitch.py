...
class ExampleSwitch13(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        if count==0:
            ...
            print packet_base.PacketBase.parser(msg.data)
            pack=packet_base.PacketBase.parser(msg.data)
            sys.stdout.flush()
            sys.stdout.write('\t\tTCP Info: ')
            print tcp.tcp.parser(msg.data)
            sys.stdout.flush()
            ...
            x[str(num)].append({ 
                ...
                'Headers' : str(eth_pkt),
                'Datafield' : str(pack),
                'TCP Info' : str(tcp.tcp.parser(msg.data)),
                'Protocols' : str(p),
                'Datapath ID' : str(dpid),
                'Port' : str(in_port)
            })
            with open('Packet_info.txt','w') as out:
                json.dump(x,out)                                #make
                                                        #the dict the 
                                                       #entire packet
                                                       #info
            num+=5
